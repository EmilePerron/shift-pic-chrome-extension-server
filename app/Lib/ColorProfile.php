...
class ColorProfile
{
    ...
    public static function ApplyFromTo($sourcePath, $applyPath)
    {
        if (exif_imagetype($sourcePath) == IMAGETYPE_JPEG && exif_imagetype($applyPath) == IMAGETYPE_JPEG) {
            $colorProfile = new ColorProfile();
            $colorProfile->LoadFromJPEG($sourcePath);
            $colorProfile->SaveToJPEG($applyPath);
        }
    }
